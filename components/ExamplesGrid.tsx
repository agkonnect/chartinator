...
export default function ExamplesGrid() {
  ...
  return (
    <>
        ...
        {CATEGORIES.map((cat) => (
          <button
            ...
            className={`px-3 py-1 text-xs rounded-full border transition-all cursor-pointer ${
              active === cat
                ? 'border-[#00D4FF] text-[#00D4FF] bg-[#00D4FF]/10 font-semibold'
                : 'border-[#1e3a5f] text-[#94a3b8] hover:border-[#00D4FF]/40 hover:text-[#e2e8f0]'
            }`}
          >
            {cat}
          </button>
      ...
      {/* Result count */}
      <p className="text-center text-xs text-[#475569] mb-6">
        {filtered.length} indicator{filtered.length !== 1 ? 's' : ''}{active !== 'All' ? ` in ${active}` : ''}
      </p>
